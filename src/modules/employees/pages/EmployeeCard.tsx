...
import AddEmployeeModal from '../components/EmployeeModal.tsx';
...
const EmployeeCard: React.FC = () => {
   ...
   const filteredEmployees = useMemo(() => {
      return employees.filter((emp) => {
         ...
         const matchDept = filterDepartment === 'all' ? true : emp.department === filterDepartment;
   ...
   const handleAddEmployee = (employee: Employee) => {
      if (editingEmployee) {
         setEmployees((prev) =>
            prev.map((item) => (item.id === editingEmployee.id ? { ...employee, id: editingEmployee.id } : item)),
         );
         ...
         render: (_: string, record: Employee) => (
            <div className="flex items-center gap-3">
               {currentPerm?.permissions.view && (
                  <button onClick={() => handleView(record)} className="hover:text-green-700 transition-colors">
                     <Eye size={18} />
                  </button>
               )}
               {currentPerm?.permissions.edit && (
                  <button onClick={() => handleEdit(record)} className="hover:text-blue-700 transition-colors">
                     <Pencil size={18} />
                  </button>
               )}
               {currentPerm?.permissions.delete && (
                  <button onClick={() => handleDelete(record)} className="hover:text-red-700 transition-colors">
                     <Trash2 size={18} />
                  </button>
               )}
